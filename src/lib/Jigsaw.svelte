<script>
	...
	function handleMousedown(x, y, e = null) {
		...
		if (movingPuzzle) {
			...
			let piece = puzzlePieces.splice(
				puzzlePieces.findIndex((p) => p.j == movingPuzzle.j && p.i == movingPuzzle.i),
				1
			);
	...
	function handleMouseup() {
		...
		if (movingPuzzle) {
			...
			const movingElementIndex = puzzlePieces.findIndex(
				(p) => p.j == movingPuzzle.j && p.i == movingPuzzle.i
			);
			if (
				Math.abs(movingPuzzle.currentPos.x - movingPuzzle.correctPos.x) < distThreshold &&
